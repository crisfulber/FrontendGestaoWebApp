...
export class PeriodoComponent implements OnInit {
  ...
  ngOnInit() {
    const configuracao = this.configService.getConfiguracao('periodo');
    if (configuracao) {
  ...
  onEditarItem(item: any) {
    if (item && item.idperiodo) {
      this.router.navigate([`/${this.endpoint}/editar/${item.idperiodo}`]);
    }
  }

  onExcluirItem(item: any) {
    if (confirm(`Deseja realmente excluir o item "${item.nome}"?`)) {
      this.baseService.delete(this.endpoint, item.idperiodo).subscribe({
